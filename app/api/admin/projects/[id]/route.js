...
export async function GET(_req,{params}) {
  ...
  const land=await pool.query("SELECT * FROM project_land_certificates WHERE project_id=$1 ORDER BY created_at DESC",[id]);
  ...
  return NextResponse.json({project:p.rows[0],media:media.rows,documents:docs.rows,landCertificates:land.rows});
}

export async function PATCH(req,{params}) {
  const user=await requireAdmin(); if(!user)return NextResponse.json({error:"Không có quyền"},{status:403});
  await initDb(); const {id}=await params; const b=await req.json();
  const keys=["name","code","developer","legal_name","project_type","status","visibility","province","district","address","latitude","longitude","price_from","price_to","currency","legal_status","progress","summary","description","amenities","project_facts","legal_checklist","progress_timeline","ai_profile","featured"];
  const sets=[],vals=[]; for(const k of keys){if(Object.prototype.hasOwnProperty.call(b,k)){vals.push(["amenities","legal_checklist","progress_timeline"].includes(k)?(Array.isArray(b[k])?b[k]:[]):["project_facts","ai_profile"].includes(k)?(b[k]&&typeof b[k]==="object"?b[k]:{}):b[k]);sets.push(`${k}=$${vals.length}`)}}
